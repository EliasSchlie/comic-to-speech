...
# Initialize Google Cloud clients (credentials should be set by now)
if credentials_available:
    try:
        ...
        print("   Please verify your credentials.json file is valid")
        vision_client = None
        tts_client = None
else:
    vision_client = None
...
class ComicOCR:
    ...
    def extract_text(self, image_bytes, preprocess=True):
        ...
        if vision_client is None:
            raise Exception("Google Vision client not initialized. Please check your credentials.")
...
def generate_audio():
    ...
    try:
        ...
        if tts_client is None:
            return jsonify({"error": "Text-to-Speech client not initialized. Please check your credentials."}), 500
...
if __name__ == '__main__':
    ...
    if credentials_available and vision_client and tts_client:
        print(f"✓ Server ready to start")
        print(f"✓ Google Cloud APIs initialized successfully")
    else:
